...
async function runScreener(profileArgs = [], timeoutMs = 180000) {
  ...
  const hbPath = outPath.replace('.json', '.hb');

  return new Promise((resolve, reject) => {
    const p = spawn(BIN, args, { 
      env: { ...process.env, JSON_OUT_PATH: outPath, HEARTBEAT_PATH: hbPath, HEARTBEAT_MS: '5000' },
    ...
    // Watchdog to escalate to SIGKILL if heartbeat stale after SIGTERM
    const watchdog = setInterval(() => {
      try {
        if (fs.existsSync(hbPath)) {
          const ts = Number(fs.readFileSync(hbPath, 'utf8'));
          if (ts && Date.now() - ts > 15000) { // >15s stale
            p.kill('SIGKILL');
    ...
    p.on('close', (code, signal) => {
      ...
      // Prefer the clean JSON file (accept even if killed by signal)
      try {
        if (fs.existsSync(outPath)) {
          const raw = fs.readFileSync(outPath, 'utf8');
          const result = JSON.parse(raw);
          if (signal) {
            console.log(`[runScreener] Accepting ${Array.isArray(result) ? result.length : 'results'} despite ${signal} signal`);
          }
          cleanup();
