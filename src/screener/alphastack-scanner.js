...
async function applyLiquidityFilter(tickers) {
    ...
    // Sample a random subset to determine adaptive threshold (avoid alphabetical bias)
    const sampleSize = Math.min(200, tickers.length);
    const shuffledTickers = shuffleArray([...tickers]); // Shuffle first to avoid alphabetical bias
    const sample = shuffledTickers.slice(0, sampleSize);
    ...
    if (volumeData.length === 0) {
        console.log('⚠️ No volume data available, returning shuffled price-filtered subset');
        return shuffleArray(tickers).slice(0, 100); // Return random 100, not alphabetical
    }
    ...
    if (filtered.length < 10) {
        ...
        // Sort by dollar volume descending, not alphabetical
        return relaxedFiltered.sort((a, b) => b.medianDollarVol - a.medianDollarVol).slice(0, 100);
    }
    
    // Sort by dollar volume descending, not alphabetical 
    return filtered.sort((a, b) => b.medianDollarVol - a.medianDollarVol).slice(0, 150);
}
...
async function runCatalystOnlyFallback(universe, config) {
    ...
    // Shuffle first to avoid alphabetical bias, then take top by liquidity
    ...
    for (const ticker of sample) {
        try {
            ...
            if (catalystData.score >= 3 || catalystData.hasVerifiedCatalyst || news.length >= 2) {
                ...
                // Add catalyst age for ranking
                const catalystAge = calculateCatalystAge(news);
                ...
                results.push({
                    ...
                    catalyst_age_hours: catalystAge,
...
function calculateCatalystAge(news) {
    ...
    for (const article of news) {
        ...
        if (article.published_utc) {
            articleTime = new Date(article.published_utc).getTime();
        } else if (article.time_published) {
            articleTime = new Date(article.time_published).getTime();
        } else if (article.published) {
            articleTime = new Date(article.published).getTime();
        } else {
            continue;
        }
        
        const ageHours = (now - articleTime) / (1000 * 60 * 60);
        if (ageHours < newestAge) {
            newestAge = ageHours;
        }
    }
    
    return Math.max(0, newestAge);
}
